...
interface FirestoreUserStatsRepository {
     ...
     * @param timestamp Timestamp de fin de séance (Date, default = now)
     * @return Result.success si mis à jour, Result.failure si erreur
     *
     * FIX(build-debug-android): Changed timestamp from Long to Date for Firestore Timestamp compatibility
     */
    suspend fun incrementSession(
        uid: String,
        durationMinutes: Int,
        timestamp: Date = Date() // FIX(build-debug-android): Changed from Long to Date
    ): Result<Unit>
